...
namespace svo
{
  namespace feature_detection_utils
  {
    ...
    SegmentAbstractDetector::Ptr makeSegmentDetector(const SegmentDetectorOptions& options,
    const CameraPtr& cam)
    {
        SegmentAbstractDetector::Ptr detector;
      switch (options.detector_type)
      {
                      ...
                      OccupandyGrid2D &grid)
    {
      ...
      for (const Corner &c : corners)
      {
        if (c.score > threshold)
        {
          ...
          grid.occupancy_[grid.getCellIndex(c.x, c.y)] = true; // in this step just for anther feature detector don't detect feature in same cell
        ...
        std::unique_ptr<upm::ELSED> &elsed_)
    {
      ...
      {
        const int scale = (1 << level);
        upm::SalientSegments salient_segs = elsed_->detectSalient(img_pyr[level]);
        ...
        for (const auto &score_seg_item : salient_segs)
        {
          ...
          y1 = score_seg_item.segment[3];
          score = score_seg_item.salience;
    ...
    void fillSegment(
        ...
        double threshold)
    {
      CHECK_EQ(keysegs.cols(), levels.size());
      CHECK_EQ(keysegs.cols(), scores.size());
      CHECK_EQ(static_cast<size_t>(keysegs.cols()), types.size());
      ...
      for (const auto  &score_segment : segs)
      {
        if (score_segment.score_ > threshold)
        {
          ...
          // feature extraction.
          if (!mask.empty() &&
              mask.at<uint8_t>(static_cast<int>(score_segment.y0), static_cast<int>(score_segment.x0)) == 0 && 
              mask.at<uint8_t>(static_cast<int>(score_segment.y1), static_cast<int>(score_segment.x1)) == 0 )
          {
            continue;
          }
          ...
          grid.feature_occupancy_[grid.getCellIndex(score_segment.x0, score_segment.y0)] = Keypoint(score_segment.x0, score_segment.y0);
        }
      }
